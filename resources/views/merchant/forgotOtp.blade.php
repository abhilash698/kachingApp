        ...
        <div class="p-l-50 m-l-20 p-r-50 m-r-20 p-t-50 m-t-30 sm-p-l-15 sm-p-r-15 sm-p-t-40">
          {!! Html::image('assets/img/custom/index.png', 'logo' , array('class' => '','width'=>'130')) !!}
           ...
           @if (session('message'))
            ...
            @endif
          <!-- START Login Form -->
          <form id="form-login" class="p-t-15" role="form" method='POST' action="/merchant/forgot/otp">
            {!! csrf_field() !!}
            <input type='hidden' name='mobile_token' value='{{$mobile_token}}'>
            <!-- START Form Control-->
            <div class="form-group form-group-default">
              <label>OTP <span class='error-msg'>{{ $errors->first('otp') }}</span></label>
              <div class="controls">
                <input type="text" name="otp" placeholder="OTP" class="form-control" required>
              </div>
            </div>
            <button class="btn btn-primary btn-cons m-t-10" type="submit">Verify</button>
          </form>
          <!--END Login Form-->
